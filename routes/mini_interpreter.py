...
class interpreter:
    ...
    def puts(self, *message):
        ...
        for i, val in enumerate(message):
            if val in self.arguments.keys():
                message[i] = self.arguments[val]
        return message[0]
    ...
    def division(self, *arg):
        ...
        if (ls[1] == "0"):
            return False
        if ('.' not in ls[0] and '.' not in ls[1]):
            return str(int(ls[0])/int(ls[1]))
        return f"{float(ls[0])/float(ls[1]):.4g}"
    ...
    def interpreter_func(self, input):
        ...
        while index < len(input):
            ...
            elif input[index] == ' ' and not inWord:
                if current_statement != "":
                    statement.append(current_statement)
                    current_statement = ""
                index += 1
...
#                   ]
    ...
    # "expressions":['(set v \"HelloWorld\")', '(puts (concat v \",Student\"))']
